...
public class SecureAgentInterService<Triplet> extends BaseService {
    ...
    private Sink senderSink=new CommandSourceSink();
    private Sink receiverSink=new CommandTargetSink();
    private Filter myOutgoingFilter = null; // We do not have outgoing filter
    private Filter myIncomingFilter = null; // We do not have Incoming filter
    private Service.Slice localSlice=new ServiceComponent();
    ...
    public void boot(Profile pf) throws ServiceException{
        myProfile=pf;
    }

    public Class getHorizontalInterface() {
        return SecureAgentInterSlice.class;
    }
    ...
    public Filter getCommandFilter(boolean direction) {
        if (direction == Filter.OUTGOING) {
            return myOutgoingFilter;
        }
        else {
            return myIncomingFilter;
        }
    }
    ...
    public String getName() {
        return SecureAgentInterHelper.NAME;
    }


    public Behaviour getAMSBehaviour() {
        //myATools=AttestTool.getInstace();
        // Get the AMS from the platform
        AID amsAID = new AID("ams", false);
        Agent ams = contenedor_actual.acquireLocalAgent(amsAID);
        // Create the message template
		/*MessageTemplate mt =
			MessageTemplate.and(
					MessageTemplate.not(MessageTemplate.MatchSender(amsAID)),
					MessageTemplate.or(
							MessageTemplate.MatchOntology(SecureMigrationService.ATTEST_REQUEST),
							MessageTemplate.MatchOntology(SecureMigrationService.ATTEST_DATA)));
		*/
        MessageTemplate mt =
                MessageTemplate.and(
                        MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
                        MessageTemplate.or(
                                MessageTemplate.MatchOntology(SecureAgentInterHelper.REQUEST_ATTESTATION),
                                MessageTemplate.MatchOntology(SecureAgentInterHelper.REQUEST_CONFIRMATION)));
        // Create the behaviour
        AMSREResponder resp = new AMSREResponder(ams, mt,myProfile, contenedor_actual.here());
        // Release the AMS agent
        contenedor_actual.releaseLocalAgent(amsAID);
        return resp;
    }



    public class SecureAgentInterHelper implements jade.core.SecureTPM.INTER.SecureAgentInterHelper{
        ...
        public synchronized void doSecureMove(SecureAgent agente, Location destino) {
            ...
            if (lista_peticiones_movimiento.size()!=0 || lista_peticiones_clonacion.size()!=0) {
                ...
                GenericCommand comando = new GenericCommand(SecureAgentInterHelper.REQUEST_ATTESTATION,SecureAgentInterHelper.NAME,"PETICION DE ATESTACION ENVIADA");
        ...
        public synchronized void doSecureClone(SecureAgent agente, Location destino, String nombre_agente) {
            /**
             En base a las limitaciones de componentes, se ha definido el comportamiento
             en principio monohilo, es decir, solo se puede realizar una operación de
             movimiento o clonación para los agentes que componen la plataforma.
    ...
    private class CommandTargetSink implements Sink {
        ...
        public void consume(VerticalCommand cmd) {
            try {
                ...
                if(name.equals(jade.core.SecureTPM.INTER.SecureAgentInterHelper.REQUEST_ATTESTATION)) {
                    ManejadorPeticionAtestacion(cmd);
                }
                else if(name.equals(jade.core.SecureTPM.INTER.SecureAgentInterHelper.REQUEST_CONFIRMATION)) {
        ...
        private void ManejadorPeticionAtestacion(VerticalCommand comando) throws ServiceException {
            ...
            ams.addBehaviour(new AMSREInitiatorRequest(ams, mensaje, peticion_confirmacion, SecureAgentInterService.this));
            contenedor_actual.releaseLocalAgent(amsAID);
    ...
    private class CommandSourceSink implements Sink {
        ...
        public void consume(VerticalCommand comando) {
            ...
            try {
                ...
                if (tipo_peticion.equals(SecureAgentInterHelper.REQUEST_ATTESTATION)) {
        ...
        public VerticalCommand serve(HorizontalCommand comando){
            ...
            try{
                ...
                if(comando.getName().equals(SecureAgentInterSlice.REMOTE_REQUEST_ATTESTATION)){
                    ...
                    comando_recibido = new GenericCommand(SecureAgentInterHelper.REQUEST_ATTESTATION,SecureAgentInterHelper.NAME,"Peticion recibida");
                    ...
                    comando_recibido = new GenericCommand(SecureAgentInterHelper.REQUEST_CONFIRMATION,SecureAgentInterHelper.NAME,"Peticion recibida");
                    comando_recibido.addParam(paquete);
                }
            }catch(Exception e){
                e.printStackTrace();
